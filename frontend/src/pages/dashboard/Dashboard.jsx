...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      setChange(Number(statsData.change || 0));
      setEsg(Number(statsData.esg || 0));
      setLoading(false);
    };
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatsCard title="Bank Balance" value={`₹${wallet.bankBalance}`} />
        <StatsCard title="Trading Balance" value={`₹${wallet.tradingBalance}`} />
        <StatsCard title="Goal Balance" value={`₹${wallet.goalBalance}`} />
      </div>
...
//             </button>
//             {saveMsg && (
//               <span className="ml-2 text-sm text-green-600">{saveMsg}</span>
//             )}
//           </form>
